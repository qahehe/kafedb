...
if [ -z $dex_variant ]; then
	echo "Usage: tpch_predicates.sh <dex_variant> <reps>"
	exit 123
fi
...
for i in $(eval echo {1..$reps}); do
	...
	./bin/spark-submit --master local[*] \
			   ...
			   --class org.apache.spark.examples.sql.dex.TPCHPredicates ./examples/target/scala-2.11/jars/spark-examples_2.11-2.4.0.jar \
			   $dex_variant \
			   &> /data/log-predicates-$dex_variant-$i
done
